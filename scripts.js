class Cuttt {
  ...
  function changeChildren(type) {
    let element;
    if (type === 'text') {
      element = '<p data-for-cut>If you try to cut my lifeline💙😁\nKeep it up !🫰🙃</p>'
    } else if (type === 'input') {
      element = '<input data-for-cut type="text"></input>'
    } else if (type === 'image') {
      element = '<img data-for-cut src="https://iili.io/Jad5MUx.png">'
    }
  ...
  function initInputText(){
    container.children[0].value=container.children[1].value='If you play Scissors,Paper🙃'
